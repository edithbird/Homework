...
In order to get a similiar result for a moving average with a short window span and a simple exponential smoothing forecast, the *SES* needs an $\alpha$ value that is very close to 1. A high $\alpha$ value translates to a fast learning model meaning the most recent values have the most impact on the forecast and past values are less important. 

**Below is a representation of the actual Amtrak ridership series and the training period fit of the moving average with a window of 3, and the simple exponential smoothing model with an alpha of 0.98. The results are ver similiar.**
...
plot(c(1991, 2004), yrange, type = "n", xlab = "Year", ylab = "Ridership", bty = "l", xaxt = "n", yaxt = "n")
lines(ridership.ts, bty = "l")
...
axis(2, at = seq(1000, 3000, 100), labels = format(seq(1000, 3000, 100)), las = 2)
lines(forecast.ma.trailing$fitted , col = "blue"
      #, lty = 2
      )
lines(ses.pred$fitted, col = "red", lty = 3)
legend(1999, 1600, c("ridership", "trailing moving average", "SES alpha = 0.98"), lty = c(1, 1, 3), col = c("black", "blue", "red"), bty = "n")
```
...
* **Moving average of raw series:** This is not a method for forecasting the Dept. Store Sales raw series because, without adjustments to the data, it can not not accomodate trend or seasonality, 2 components in this data series.  

* **Moving average of deseasonalized series:** Moving average forecasting is for data that demonstrates stationarity, it should not have trend or seasonality. The six year series of department store quarterly sales has a seasonality component and perhaps a trend component. Deseasonalizing the series is essential when forecasting with this method. Deseasonalized data may well be adquate for forecasting with the moving average because there is evidence to support the lack of an impactful trend component. Because we can not conclude a lack of trend with certainty, trial and error is required. If there is no trend, data that is deseasonalized with differencing can be used to forecast with a moving average. Deseasonalizing quarterly data uses a lag of 4. 

Below are plots of the actual Dept Store Sales, the deseasonalized series, the detrended series, and the deseasonalized, detrended series. 
