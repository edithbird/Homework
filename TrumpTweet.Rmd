...
```{r}
trump_tweets <- userTimeline("POTUS", n = 5000)
...
tweets <- tweets %>% select(created, text, retweetCount) %>% group_by(created) %>% summarise(Retweets = sum(retweetCount))
kable(tweets)
```


```{r}
tweets %>% ggplot(aes(created, Retweets)) + geom_col(fill ="#CC0000") + labs(x = "Day", y = "Number of Retweets") 
